# パターン C: Blazor 一括インライン編集

## 概要

一覧テーブルの各行を直接編集し、画面上の **1 つの「更新」ボタン** で追加・更新・削除をまとめて保存するパターンです。  
Blazor の状態管理で差分を検出し、UnitOfWork によるトランザクションで一括コミットします。

**採用条件**:
- 複数行を一度に変更して一括保存したい（管理画面・設定画面など）
- 画面遷移なしで CRUD を完結させたい
- 行の追加・削除・編集が混在し、すべて成功か全ロールバックにしたい

**実装例**: `CategoryList.razor`, `MyTodo.Web/BlazorComponents/Categories/`
...
MyTodo.Application/
  Repositories/
    IUnitOfWork.cs              ← トランザクション制御インターフェース
    ICategoryRepository.cs      ← SaveChanges なしのリポジトリ IF
  Queries/Categories/
    ICategoryQueryService.cs
    CategoryReadModel.cs
  Commands/Categories/
    SaveCategoriesCommand.cs    ← 追加/更新/削除 を 1 コマンドにまとめる + Handler
...
MyTodo.Web/
  ...
  Controllers/
    CategoriesController.cs     ← View のみ返す（データを持たない）
  Views/Categories/
...
### 1. Application 層 — UnitOfWork インターフェース
...
### 2. Application 層 — Repository インターフェース（SaveChanges を呼ばない）

UnitOfWork 対応リポジトリは戻り値を `Task`（void 相当）にし、`SaveChangesAsync()` を呼びません。  
EF Core の ChangeTracker に操作を積むだけにします。
...
通常リポジトリ（`ITodoRepository` 等）との比較:
...
### 3. Application 層 — ReadModel & QueryService インターフェース

```csharp
// MyTodo.Application/Queries/Categories/CategoryReadModel.cs
public record CategoryReadModel(int Id, string Name, DateTime CreatedAt);
```

```csharp
// MyTodo.Application/Queries/Categories/ICategoryQueryService.cs
...
### 4. Application 層 — Command & Handler（一括保存）

追加・更新・削除を **1 つの Command** で表現します。

```csharp
// MyTodo.Application/Commands/Categories/SaveCategoriesCommand.cs

// 更新対象行の表現（id + 新しい値）
public record CategoryChange(int Id, string Name);

// 追加/更新/削除 の 3 リストを 1 コマンドにまとめる
...
### 5. Infrastructure 層 — EfUnitOfWork 実装
...
### 6. Infrastructure 層 — リポジトリ実装（SaveChanges を呼ばない）

```csharp
// MyTodo.Infrastructure/Repositories/EfCategoryRepository.cs
public class EfCategoryRepository : ICategoryRepository
{
    ...
    public async Task UpdateAsync(int id, string name)
    {
        ...
        // ChangeTracker が Modified として追跡（SaveChanges は呼ばない）
    }
    ...
    public async Task DeleteAsync(int id)
    {
        ...
        // ChangeTracker が Deleted として追跡（SaveChanges は呼ばない）
    }
}
```
...
### 7. Infrastructure 層 — QueryService

```csharp
// MyTodo.Infrastructure/Queries/CategoryQueryService.cs
...
### 8. Web 層 — Blazor コンポーネントの行状態モデル（RowModel）

コンポーネント内に `private class RowModel` を定義し、各行の UI 状態を管理します。
...
private class CategoryRowModel
{
    ...
    public bool IsNew { get; set; }              // DB に未保存の行
...
### 9. Web 層 — Blazor コンポーネント全体

```razor
@* MyTodo.Web/BlazorComponents/Categories/CategoryList.razor *@
...
### 10. Web 層 — MVC ホストコントローラー & View

```csharp
// MyTodo.Web/Controllers/CategoriesController.cs
...
```cshtml
@* MyTodo.Web/Views/Categories/Index.cshtml *@
@{
...
<component type="typeof(MyTodo.Web.BlazorComponents.Categories.CategoryList)"
